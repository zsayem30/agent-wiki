...
long sources. After editing, run focused verification and leave a curator
handoff with scan_changes.py if durable truth may have changed.
```
...
```text
Please review the latest handoff. Use:

python agent-wiki/scripts/wiki/handoff_queue.py --project-root . next

Update compact wiki truth only where evidence supports a durable change. Put
ambiguity in OPEN_QUESTIONS.md. Run python agent-wiki/scripts/wiki/lint.py
before finishing.
```
